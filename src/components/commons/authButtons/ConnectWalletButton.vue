...
export default {
  ...
  computed: {
    ...
    selectedBlockchainLogo() {
      return getCosmosCoinLogo(
        `${this.getOnChainIssuerConfig.ecosystem}:${this.getOnChainIssuerConfig.blockchain}:${this.getOnChainIssuerConfig.chainId}`
      )
    },
  },
  methods: {
    ...
    async connectWallet() {
      ...
      const { default: SupportedChains } = await import(
        `@hypersign-protocol/hypersign-kyc-chains-metadata/${ecosystem}/wallet/${blockchain}/${this.getOnChainIssuerConfig.chainId}/chains`
      )
      ...
      if (!chainConfig) {
        throw new Error(
          'Chain not supported for chainId requestedChainId ' + requestedChainId
        )
      }
