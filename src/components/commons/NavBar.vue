  ...
  <div class="navbar navbar-expand-md navbar-light">
    <div class="container">
      <a href="#" class="navbar-brand">
        <img
          class=""
          src="../../../public/assets/img/hypersign_black_transparent_rect.png"
          width="100px"
        />
      </a>
      ...
      >
        {{ getActiveStep.name }} ({{
          getActiveStep.id - length - lengthToMinus
        }}/{{ kycStepsLength }})
      </span>
    </div>
  </div>
...
export default {
  computed: {
    ...
    length() {
      if (
        this.steps.find((x) => x.stepName === STEP_NAMES.ZkProofs).isEnabled ===
        false
      ) {
    ...
    lengthToMinus() {
      ...
      const notActiveSteps = allRelevantSteps.filter(
    ...
    kycStepsLength() {
      return this.steps.filter(
        (step) => step.name != null && step.isEnabled === true
      ).length
    },
  },
}
