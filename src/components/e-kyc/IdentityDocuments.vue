      ...
      <div v-else>
        <div class="row" style="text-align: left; min-height: 550px" v-if="!ifExtractedData && selectedDocumentType != ''">
          <div class="col-md-8 mx-auto" style="position: relative; min-height: 400px; max-height: 80%">
            <facephi-selphid v-if="isWidgetStarted && selectedDocumentType != ''" :licenseKey="licenseKey" :bundlePath="bundlePath" :language="language" :initialTip="initialTip" :askSimpleMode="askSimpleMode" :cameraWidth="cameraWidth" :cameraHeight="cameraHeight" :cameraSelection="cameraSelection" :previewCapture="previewCapture" :forceLandscape="forceLandscape" :captureTimeout="captureTimeout" :captureRetries="captureRetries" :imageFormat="imageFormat" :imageQuality="imageQuality" :documentType="documentType" :scanMode="scanMode" :blurredThreshold="blurredThreshold" :showLog="showLog" :debugMode="debugMode" :documentMode="documentMode" @onmoduleloaded="onModuleLoaded" @onextractionfinished="onExtractionFinished" @onusercancelled="onUserCancelled" @onexceptioncaptured="onExceptionCaptured" @onextractiontimeout="onExtractionTimeout" @ontrackstatus="onTrackStatus" :startSimpleMode="manualMode"></facephi-selphid>
          </div>
        </div>
